...
const getObjectOnClick = function (cards) {
  for (let card of cards) {
    card.addEventListener(`click`, function (e) {
      ...
      const newHtml = `                
        <div class="added-student flex flex-space-between mg-btm-05">
          <div class="flex-column">
            <span>${selectedUser.name}</span>
            <span class="font-sm">${selectedUser.level}</span>
          </div>
          <i class="ph ph-x remove-student"></i>
        </div>`;

      foundUsers.push(selectedUser)

      addedStudentsList.insertAdjacentHTML(`beforeend`, newHtml);
    });
  }
};
